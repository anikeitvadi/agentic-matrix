        ...
        {navItems.map((item) => {
          ...
          if (item.disabled) {
            return (
              <div
                key={item.name}
                className="flex items-center gap-3 px-3 py-2 rounded-lg text-neutral-500 cursor-not-allowed"
              >
                {item.icon}
                <span className="text-sm">{item.name}</span>
                {item.badge && (
                  <span className="ml-auto text-xs bg-neutral-800 text-neutral-500 px-2 py-0.5 rounded">
                    {item.badge}
                  </span>
                )}
              </div>
